...
int main()
{
	...
	while (window.isOpen())
	{
		...
		circle.setFillColor(Color(10, 0, 230)); // закрашиваем наш круг
		circle.setOutlineThickness(5.f); // устанавливаем толщину контура круга
		circle.setOutlineColor(Color(0, 0, 0)); // устанавливаем цвет контура
		...
		triangle.setPosition(250, 10); // устанавливаем начальную позицию справа от круга
		triangle.setFillColor(Color::Black); // устанавливаем цвет треугольника - синий
		window.draw(triangle); // отрисовка треугольника

		// Создаём квадрат
		CircleShape square(20.f, 4);
		square.setPosition(300, 50); // устанавливаем начальную позицию справа от треугольника
